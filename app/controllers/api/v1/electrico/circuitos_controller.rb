...
module Api
  module V1
    module Electrico
      class CircuitosController < Api::V1::ElectricoController
...
#        before_action :cache_config
        ...
        def divisas
          expires_in 1.seconds, :public => false
          m = Marketplace::Divisas.new()
          respond_to do |format|
            format.json {render json: { :dolar => m.get_dolar, :mlc => m.get_mlc  }}
          end
        end
	...
	def get
	  ...
	  @presupuesto = ::Electrico::Presupuesto.first unless @presupuesto
        ...
        def addToCircuito
          ...
          response.headers['Access-Control-Allow-Credentials'] = true
          response.headers['Access-Control-Expose-Headers'] = 'AMP-Access-Control-Allow-Source-Origin'
          @circuito = ::Designer.new(amp_params.merge({'forro' => 'THHN'})).addToCircuito          
          respond_to do |format| 
